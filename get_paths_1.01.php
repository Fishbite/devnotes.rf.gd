...
/**
 * Summary of scanDirectory
 * @param string $dir the directory to scan
 * @param string $projectRoot project root
 * @param mixed $file file pointer - file to write to
 * @return void
 */
function scanDirectory(string $dir, string $projectRoot, $file): void
{
    ...
    writePaths($items, $projectRoot, $file);

    
}

function writePaths ($items, $projectRoot, $file) {
    ...
    if ($items === false) {
      ...
      foreach ($items as $item) {
          if (is_dir($item)) {
            ...
            if (isset($pathInfo['extension']) && in_array(strtolower($pathInfo['extension']), ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg'])) {
