<main class="container">
  <article>
    ...
    <section>
      ...
      <p>Which gives us the very same form we had before, except that, if we now press the submit button without filling in any or all of the fields, a message is displayed to the user &mdash; "Please fill out this field". This is because we added the <samp>required</samp> attribute to each input element. If we try to submit a number in the <samp>age</samp> field that is less than 1, or, greater  than 150, the user recieves an additional message telling them what range of numbers the form will accept. This is <em>front-end</em> validation. It's the first form of defense against duff data.</p>
      ...
      <p>So, we simply used an <samp>echo</samp> command to print out the value of the index named<samp>fname</samp>. However, we have to take care when doing this, especially because this is raw data that has been entered into our web app'.</p>
      ...
      <p>What does this mean to us? It means that we should ensure that the data should always be made to be a <samp>string</samp> no matter what the content of that string is!</p>
      ...
      <p>We can see that our data is stored in an associative array consisting <code>key=>value</code> pairs. How can we use that data safely in an SQL query? We must use prepared statements and parameterised values. That means, preparing the SQL statement separately from the data, then, add the data to the query via a parameter in the query instead of using the variable directly, this will ensure that it can not alter the query itself. Using a prepared statement will gaurantee that our query is properly formated. There are two methods available for this purpose, PDO and MySQLi prepared statements.</code></p>

      <p>Lets take a look at how we might actually create one of these.</p>

      <h2>the format of prepared statements</h2>

      <p>Creating prepared statements can only be done once you are connected to a database. However, that is beyond the scope of this chapter, so, we'll just examine the actual structure and sequence of the two methods in this section. Database creation and utilization will be covered another article in detail.</p>
      ...
      <p>The mysqli method can only be used to query a MySQL database. It is the worlds most popular open source database, so, it's a good place to start!</p>
      <p>Further, the mysqli method will <strong>always</strong> run a prepared statement as a prepared statement, whereas, this is not true of the PDO method, in some circumstances. I know that sounds crazy, but, it's apparently the case  !!!</p>
      ...
      <ul class="in-article-list">
        <li><samp>"is"</samp> means 1st variable is an integer, 2nd is a string</li>
        <li><samp>$id, $label</samp> the real names of the variables to bind</li>
      </ul>

      <p><strong>Note:</strong> We <strong>must</strong> write the variable type descriptors <samp>i</samp> & <samp>s</samp> and the variable names in the correct sequence. i.e. <samp>i</samp> represents the first variable <samp>$id</samp> which is bound to the first placeholder <samp>?</samp> question mark. While type descriptor <samp>s</samp> represents the second variable <samp>$label</samp> and the second placeholder <samp>?</samp> question mark used in the prepared statement.</p>
    </section>
    ...
    <p>The PDO and MySQLi methods are available to create prepared statements. There are three steps to create them: prepare, bind and execute. The <code>mysqli</code> method is purely for use with a MySQL data base, while PDO can be used for any database engine.</p>
